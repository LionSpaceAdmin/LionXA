...
async function createBackup(): Promise<void> {
    try {
        ...
        const memory = JSON.parse(await fs.readFile(config.data.memory, 'utf-8'));
        const logs = JSON.parse(await fs.readFile(config.data.handledTweets, 'utf-8'));
...
export async function restoreFromLatestBackup(): Promise<void> {
    try {
        ...
        if (backupData.cookies) {
            await fs.writeFile(config.data.cookies, JSON.stringify(backupData.cookies, null, 2));
        }
        await fs.writeFile(config.data.memory, JSON.stringify(backupData.memory, null, 2));
        await fs.writeFile(config.data.handledTweets, JSON.stringify(backupData.engagementLogs, null, 2));

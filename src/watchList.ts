    ...
    try {
        ...
        const hasContent = await Promise.race([
            page.waitForSelector('article[role="article"]', { timeout: 30000 })
                .then(() => true)
                .catch(() => false),
            page.waitForSelector('[data-testid="empty-timeline"]', { timeout: 30000 })
                .then(() => false)
                .catch(() => true)
        ]);
        ...
        for (const article of articles) {
            try {
                ...
                if (username && id) {
                    ...
                    const imageLocators = await article.locator('img, [data-testid="tweetPhoto"] img').all();
                    const images: string[] = [];
...
async function postReply(tweet: Tweet, replyText: string): Promise<boolean> {
    try {
        ...
        console.log(`Waiting ${Math.round(delayMs/1000)}s before replying to @${tweet.username}...`);
...
async function main() {
    ...
    async function navigateToList(): Promise<void> {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const rateLimited = await page.locator('text="Rate limit exceeded"').count()
                    .catch(() => 0);
                ...
                await Promise.race([
                    page.waitForSelector('[data-testid="primaryColumn"]', { timeout: 20000 }),
                ...
                const content = await Promise.race([
                    page.waitForSelector('article[role="article"]', { timeout: 30000 }),
                    page.waitForSelector('[data-testid="ListHeader"]', { timeout: 30000 })
                ]);
                ...
                if (attempt === maxRetries) {
                    throw new Error(`Failed to load list after ${maxRetries} attempts: ${lastError.message}`);
                }
    ...
    while (true) {
        ...
        try {
            ...
            const url = p.url();
            if (!url.includes('x.com') || (!url.includes('/lists/') && !url.includes('/notifications'))) {
                console.log('🟡 User is interacting elsewhere; skipping scan to remain non-intrusive.');
            } else {
                ...
                for (const tweet of currentTweets) {
                    if (!isSeen(tweet.id)) {
                        console.log(`[NEW] @${tweet.username}: "${tweet.text.slice(0, 60)}..."`);
                        if (!(await canReply())) {
                            console.log('⛔ Rate limit reached for this window; skipping replies.');
                            break;
                        }
        ...
        const backoff = Math.min(POLLING_INTERVAL_MS * Math.pow(2, Math.max(0, consecutiveErrors - 1)), 5 * 60 * 1000);

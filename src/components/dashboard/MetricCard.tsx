  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const chartData = {
        labels: data
          .map((e) =>
            new Date(e.timestamp).toLocaleTimeString("he-IL", {
              hour: "2-digit",
              minute: "2-digit",
            }),
          )
        ...
        datasets: [
          {
            label: "Replies",
            data: data
              .map((e) => (e.event === "reply_posted" ? 1 : 0))
              .reverse(),
            borderColor: "#3b82f6",
  ...
  useEffect(() => {
    if (isNetworkCanvas && networkCanvasRef.current && data?.length) {
      ...
      if (event.data?.username) {
        ...
        node.className =
          "network-node absolute w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-xs font-bold animate-pulse-short cursor-pointer transition-all duration-300 hover:scale-110 shadow-lg";
      ...
      <div className="flex-1 p-4 pt-0">
        {isChart ? (
          ...
          <div
            ...
            className="network-canvas h-full min-h-[300px] relative overflow-hidden rounded-lg bg-background/60"
          >

...
// Lightweight reporting helper to the frontend API (Docker: http://frontend:3000)
async function reportToFrontend(payload: unknown) {
  const url = process.env.FRONTEND_REPORT_URL || "http://frontend:3000/api/report";
  try {
    await fetch(url, {
      method: "POST",
    ...
    console.warn("Report to frontend failed:", e);
  }
}
    ...
    if (!isSeen(tweet.id)) {
      ...
      if (profile) {
        ...
        if (reply) {
          ...
          // Report to frontend API (non-blocking best-effort)
          reportToFrontend({
            phase: "initial",
            tweetId: tweet.id,

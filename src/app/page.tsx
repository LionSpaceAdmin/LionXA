...
export default function Home() {
  ...
  const [screencap, setScreencap] = useState<{
    image: string;
    url?: string;
    ts: number;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    newSocket.on(
      ...
      (data: {
        ...
        if (typeof data.screencapMs === "number")
          setScreencapMs(data.screencapMs);
        setDiagEvents(
          (data.recentEvents || []).filter(
            (e) =>
              e.event === "error" ||
              e.event === "exception" ||
              e.event === "page_console",
          ),
        );
      },
    );
    ...
    newSocket.on(
      "screencap",
      (payload: { image: string; url?: string; ts: number }) => {
  ...
  const getMetricValue = (
    event: "reply_posted" | "gemini_call",
    condition: (e: DashboardEvent) => boolean,
  ) => {
    return recentEvents.filter((e) => e.event === event && condition(e)).length;
  };

  const repliesCount = getMetricValue(
    "reply_posted",
    (e) => e.data.success === true,
  );
  const geminiCalls = getMetricValue("gemini_call", () => true);
  const successRate =
    geminiCalls > 0 ? ((repliesCount / geminiCalls) * 100).toFixed(0) : 0;
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1 text-xs">
                      ...
                      <select
                        value={screencapMs}
                        onChange={(e) =>
                          applyScreencapMs(parseInt(e.target.value))
                        }
                        className="bg-neutral-800 border border-neutral-700 rounded px-1 py-0.5 text-neutral-200"
                      >
                    ...
                    {isPaused && (
                      <span className="text-amber-400 text-xs border border-amber-500/40 rounded px-2 py-0.5">
                        מושהה
                      </span>
                    )}
                    <div className="text-xs text-neutral-500">
                      {screencap
                        ? new Date(screencap.ts).toLocaleTimeString()
                        : "—"}
                    </div>
                  </div>
                </div>
                ...
                >
                  {screencap?.image ? (
                    ...
                    <div className="text-neutral-500 text-sm">
                      אין תצוגה חיה — הפעל סוכן כדי לראות תמונה
                    </div>
                  )}
                  {needsLogin && (
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center">
                      <form
                        ...
                        className="p-6 rounded-lg border border-neutral-700 bg-neutral-800/80 flex flex-col gap-3 text-sm"
                      >
                        ...
                        <button
                          type="submit"
                          className="bg-blue-600 hover:bg-blue-700 rounded px-3 py-1.5 text-white font-semibold"
                        >
                          התחבר
                        </button>
                      </form>
            ...
            <div className="mt-8">
              <div className="rounded-lg border border-neutral-800 bg-neutral-900 overflow-hidden">
                <div className="flex items-center justify-between px-4 py-2 border-b border-neutral-800">
                  ...
                  <div className="text-xs text-neutral-500">
                    מציג שגיאות ולוגים מהדפדפן והסוכן
                  </div>
                </div>
                <div className="max-h-80 overflow-y-auto text-sm">
                  {diagEvents.length === 0 ? (
                    ...
                    diagEvents.map((ev: DashboardEvent, idx: number) => {
                      ...
                      const level =
                        ev.data.level ||
                        (ev.event === "error" || ev.event === "exception"
                          ? "error"
                          : "info");
                      const color =
                        level === "error"
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className={`text-xs mr-2 ${color}`}>
                            [{level}]
                          </span>

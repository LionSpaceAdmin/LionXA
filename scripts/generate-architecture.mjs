#!/usr/bin/env node
// Generate architecture snapshot for aritcector/architecture.data.js (no server needed)
...
const OUT = path.join(ROOT, 'aritcector', 'architecture.data.js');
...
function resolveImport(fromFile, imp) {
  ...
  const tryPaths = [imp, imp + '.ts', imp + '.tsx', imp + '.js', imp + '.jsx', imp + '/index.ts', imp + '/index.tsx'];
...
function collect() {
  const metas = [];
  let totalFiles = 0, totalLines = 0, tsFiles = 0;
  for (const d of SCAN_DIRS) {
    ...
    for (const f of files) {
      ...
      const imps = parseImports(code).map(i => resolveImport(f, i)).filter(Boolean);
  ...
  const sections = [
    ...
    { id:'scripts', title:'scripts/', description:'שירותים (למשל Lighthouse)' }
  ];
...
function ensureDirOf(p){ try { fs.mkdirSync(path.dirname(p), { recursive: true }); } catch {} }
function writeOut(data){
  const header = `// AUTO-GENERATED. Do not edit by hand.\n`;
  const body = `window.XAGENT_ARCH = ${JSON.stringify(data, null, 2)};\n`;
  ensureDirOf(OUT);
  fs.writeFileSync(OUT, header + body, 'utf8');
  console.log(`✅ Wrote snapshot: ${rel(OUT)} (files=${data.stats.totalFiles}, lines=${data.stats.totalLines})`);
}
...
function main(){
  ...
  const run = debounce(()=>{ try { writeOut(collect()); } catch(e){ console.error('Error:', e); } }, 300);

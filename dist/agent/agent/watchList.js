...
const LIST_URL = config_1.config.twitter.listUrl;
const TARGET_USERNAMES = new Set(Array.from(config_1.config.twitter.targetUsernames).map(u => u.toLowerCase()));
const POLLING_INTERVAL_MS = config_1.config.agent.pollingIntervalMs;
const INITIAL_REPLY_DELAY_MS = config_1.config.agent.initialReplyDelayMs;
const ACTION_TIMEOUT_MS = config_1.config.agent.actionTimeoutMs;
const INITIAL_POST_LIMIT = config_1.config.agent.initialPostLimit;
 ...
 */
async function scrapeTweetsFromList(page) {
    const tweets = [];
    try {
...
async function postReply(page, tweet, replyText) {
    ...
    try {
        ...
        (0, logging_1.logHandledTweet)(tweet.id, replyText);
...
async function main() {
    ...
    (0, memory_1.registerGracefulShutdown)();
    const { page } = await (0, browser_1.ensureSession)();
    // Initial navigation
    try {
    ...
    while (true) {
        try {
            ...
            for (const tweet of currentTweets) {
                const seen = await (0, memory_1.isSeen)(tweet.id);
                if (!seen) {
                    console.log(`[NEW] New post from @${tweet.username}: "${tweet.text.slice(0, 80)}..."`);
                    const profile = await (0, profile_manager_1.getProfileByHandle)(tweet.username);
                    if (profile) {
                        ...
                        const reply = await (0, gemini_1.askGemini)(prompt);
                        if (await postReply(page, tweet, reply)) {
                            await (0, memory_1.markSeen)(tweet.id);
                        ...
                        await (0, memory_1.markSeen)(tweet.id); // Mark as seen to avoid re-processing
                    }
                }
            }
        }
